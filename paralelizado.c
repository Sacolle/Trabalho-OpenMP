...
void calcula_media(double *matriz, double *vet,int lin, int col){
    int i,j;
    double soma;
	#pragma omp parallel for schedule(auto) private(i, j, soma)
    for(i=0;i<col;i++){
        soma=0;
        for(j=0;j<lin;j++){
            soma+=matriz[i*lin+j];
...
int main(int argc,char **argv){
    ...
    double *matriz, *matriz_ord,*mediana,*media,*media_har,*moda,*variancia,*dp,*cv; // Define a matriz (forma linear), vetores de medidas estatísticas
    ...
    matriz_ord=(double *)malloc(lin*col * sizeof(double)); // Aloca a matriz
    ...
    for(i=0;i<lin;i++){
        for(j=0;j<col;j++){
            fscanf(stdin, "%lf ",&inp); // Lê os dados transpostos em uma matriz de entrada
			matriz[j*lin+i] = inp;
			matriz_ord[j*lin+i] = inp;
        }
    }
	#pragma omp parallel sections
	{
		...
		#pragma omp section
		ordena_colunas(matriz_ord, lin, col);
	}
	...
	#pragma omp parallel sections
	{
		...
		#pragma omp section
		calcula_mediana(matriz_ord,mediana,lin,col);
		#pragma omp section
		calcula_moda(matriz_ord,moda,lin,col);
	}
