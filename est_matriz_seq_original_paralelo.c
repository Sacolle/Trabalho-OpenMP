...
void ordena_colunas(double *matriz, int lin, int col) {
    ...
    for (i = 0; i < col; i++) {    
        // Manda o endereço do primeiro elemento da coluna
...
void calcula_media(double *matriz, double *vet, int lin, int col){
    ...
    #pragma omp parallel for private(j, soma)
    for(i = 0; i < col; i++){
        soma = 0;
        for(j = 0; j < lin; j++){
            soma += matriz[i*lin + j];
...
double moda_aux(double *matriz, int lin){
    ...
    for(i = 0; i < lin; i++){
        cont[i] = 0;  // Initialize the count array
    }
...
int main(int argc, char **argv){
    int lin, col, i, j; // Variáveis de índices e dimensões
    double *matriz, *mediana, *media, *media_har, *moda, *variancia, *dp, *cv; // Vetores de medidas estatísticas
    ...
    media = (double *)malloc(col * sizeof(double)); // Aloca o vetor de média
    media_har = (double *)malloc(col * sizeof(double)); // Aloca o vetor de média harmônica
